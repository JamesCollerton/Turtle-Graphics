...
// from the WhiteSpace, whereas this is used to open the file and directly scan in the contents. It opens
...
void open_whitespace_program_file_verbatim(char *filename, whitespace_info *whitespace_prog)
{
	...
	printf("\nScanned in whitespace components verbatim.\n");
}
...
void write_combination_to_file(program *prog, whitespace_info *whitespace_prog)
{
	...
	printf("\nEnd of point where files have the same number of lines.\n");

	if(whitespace_prog -> ws_num_lines > current_line){

		printf("\nRegular file detected to have had more lines than whitespace file.\n");
		...
		printf("\nRegular file detected to have had less lines than whitespace file.\n");
...
void print_only_ws_to_file(whitespace_info *whitespace_prog, FILE **combined_file, int current_line)
{
	...
	printf("\nOnly whitespace printed to file.\n");
}
