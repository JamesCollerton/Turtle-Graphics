...
// etc. and then parses from there. If none of these are registered then there is an error in the input and this is noticed
// and printed.
...
void printing(player *user, printing_board *first_board, SDL_Simplewin *sw)
{
	...
	do{
		Neill_SDL_Events(sw);
	}while(!sw -> skip_checker && !sw->finished);
}
...
// Goes through one board, saves where the next one is, then frees the current one. Does this until
// no boards are left.
