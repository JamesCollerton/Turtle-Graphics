// Functions mainly involved in interpreting WhiteSpace.
...
void scan_in_whitespace_components(whitespace_info *whitespace_prog, char ch)
{
	...
	// Looking for a number indicator.
	if(first_pass == on){
		if(ch == '\t'){
...
// the WhiteSpace representation. Note that WhiteSpace only deals with integers!
...
void write_prog_to_whitespace_file(program *reg_prog, FILE **prog_file)
{
	...
	for(i = 0; i < reg_prog -> num_lines_in_file; ++i){
		...
		if(sscanf(reg_prog -> prog_line[i], "%d", &temp_num) != NO_INTEGER){
