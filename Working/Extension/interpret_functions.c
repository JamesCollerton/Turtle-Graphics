...
// helpful. However it does help a bit and makes a lot of animations for my given pictures look a bit nicer (try pseudo_torus.txt at 500).
...
// Then if we are at an iteration as specified by the user in their animation speed we add a board to the linked
// list to be printed.
...
// This function is called and adds boards to a linked list every time the user has instructed. 
// A board is malloced and then put on the end of the latest board. The next board from there is set to NULL.
...
// This is used every time we have been through the loop to update the looping array. We add one to how many times we have been through the loop
