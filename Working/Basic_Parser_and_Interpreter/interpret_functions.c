...
// can create a smoother animation (which is not implemented here, but is in the extended).
...
void draw_next_square(int hypotenuse_length, player *user, int upper_bound, grid_location grid_loc, int number_steps)
{
	...
	if( grid_loc != top_right ){
		...
		// a and b are sides of the triangle calculated using SOHCAHTOA
...
// We interpret turning left and turning right. If the constant checker is on then we turn right or left by that
// constant amount, otherwise we turn right or left by a variable amount. I took angles to be integers as it seemed
// sensible.
...
// that amount, if we are turning right then we decrement the angle by the amount. Note that 0 degrees is 0 in cartesian
// coordinates, not by compass.
...
void op_interpret(program *prog, player *user, polish_stack *pol_stack)
{
	...
	//Operation indicator set to on, the result is put back on the top of the stack and the current place in the stack is decremented.
